...
//////////////////////
let yhck = process.env.yhck; // 这里是 从青龙的 配置文件 读取你写的变量
let yhck_dataArr = [];
let data = '';
let msg = '';
!(async () => { 
  if (!(await Envs())) //多账号分割 判断变量是否为空  初步处理多账号
    ...
    console.log(` 永辉生活APP脚本V0.0.1 `); // console.log是输出信息的，可以在脚本日志中看到输出（打印）的信息
    console.log(`\n\n=========================================    \n脚本执行 - 北京时间(UTC+8)：${new Date(
			new Date().getTime() + new Date().getTimezoneOffset() * 60 * 1000 +
			8 * 60 * 60 * 1000).toLocaleString()} \n=========================================\n`);
    await wyy();
    console.log(`\n=================== 共找到 ${yhck_dataArr.length} 个账号 ===================`)
...
function signin(timeout = 3 * 1000) {
  return new Promise((resolve) => {
    let url = {
      url: `https://api.yonghuivip.com/web/coupon/signreward/sign?platform=${data.platform}&shopid=${data.shopid}&access_token=${data.access_token}`, // 这是请求的 url 可以直接用我们抓包、精简后的URL
      headers: {
